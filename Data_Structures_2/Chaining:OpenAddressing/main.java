...
public class main {
    ...
    public static void main(String[] args) {
        ...
        for (int n : nList) {
             ...
             */
            //ADD YOUR CODE HERE
            Double loadFactor= ((double)n/(double)MyChainTable.m);
            
            
            Double sumChainCollisions=0.0; //SUM
            Double sumOpenCollisions=0.0;
            
            Double averageCollisionsChain =0.0;//AVG
            Double averageCollisionsOpen=0.0;
            
            for(int i=0;i<n;i++){//Insert n keys 
            	int key = keysToInsert[i];
            	
            	sumChainCollisions+=MyChainTable.insertKey(key);
            	sumOpenCollisions+= MyProbeTable.insertKey(key);
            }
            
            averageCollisionsChain= sumChainCollisions/n;//Find average 
            averageCollisionsOpen= sumOpenCollisions/n;
            
            alphaList.add(loadFactor);
            avColListChain.add(averageCollisionsChain); //ADD TO CVS 
            avColListProbe.add(averageCollisionsOpen);
 ...
 /* In this exercise, you apply your removeKey method on an example.
        ...
        for(int i=0;i<16;i++){
        	...
        	Double collisions =(double) MyProbeTable.removeKey(key);
 ...
 /*In this exercise, the hash tables are random with no seed. You choose
        ...
        //ADD YOUR CODE HERE
         int[] wInputs= {2,4,6,8,10,12,14,16,18};
         n=16; 
      
         for(int uniqueW: wInputs){
	        Double loadFactor =0.0;
	        Double sumCollisionsOpen=0.0;
	        Double sumCollisionsChain=0.0;
	    	  
	        for(int i=0;i<10;i++){//10 Different iterations 
	        	 Chaining MyChainTable = new Chaining(uniqueW, -1);//Create tables 
	        	 MyProbeTable = new Open_Addressing(uniqueW, -1);
	        	 
	        	 ArrayList<Integer> history =new ArrayList<Integer>();//Make history so as not to add same key twice 
	        	 loadFactor= ((double)n/(double)MyChainTable.m);
	           
	        	 	for(int j=0;j<n;j++){
	        	 		int randomNumber = generateRandom(0,700,-1);
	        	 		if(history.contains(randomNumber)){
	        	 			j--;
	        	 			continue;//gets a new number
	        	 		}
	        	 		sumCollisionsChain+= MyChainTable.insertKey(randomNumber);//Sum 
	        	 		sumCollisionsOpen+= MyProbeTable.insertKey(randomNumber);
	        	 		
	        	 		history.add(randomNumber);//Adds to history 
	        	   
	        	 	}
	        	 	
	         }
	         Double keysAdded= n*10.0;//XXXXXX
	         avColListChain2.add(sumCollisionsChain/keysAdded);
	         avColListProbe2.add(sumCollisionsOpen/keysAdded);
	         alphaList2.add(loadFactor);
      }
